...
export default function LessonObjectives({ objectives }: LessonObjectivesProps) {
  return (
    <div className="max-w-4xl mx-auto px-4 -mt-12 mb-8">
      <div className="bg-story-card rounded-2xl p-6 card-shadow border border-docker-blue/10">
        <div className="flex items-center gap-2 mb-3">
          <svg
            className="w-5 h-5 text-docker-blue flex-shrink-0"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
          ...
          <h4 className="text-sm font-semibold text-docker-blue font-mono uppercase tracking-wider">
            Learning Objectives
          </h4>
        </div>
        <ul className="space-y-2">
          {objectives.map((obj, i) => (
            <li key={i} className="flex items-start gap-2 text-sm text-text-secondary leading-relaxed">
              <span className="text-docker-blue mt-1 flex-shrink-0">&#8226;</span>
              {obj}
            </li>
          ))}
        </ul>
