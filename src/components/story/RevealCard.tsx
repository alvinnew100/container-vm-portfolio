  ...
  return (
    <div className="bg-story-card rounded-2xl p-6 card-shadow my-6">
      <div className="flex items-start gap-3 mb-4">
        <div className="w-8 h-8 rounded-full bg-docker-amber/10 flex items-center justify-center flex-shrink-0">
          <svg className="w-4 h-4 text-docker-amber" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {!revealed ? (
            ...
            >
              ...
              {hint && (
                <div className="mb-3">
                  {showHint ? (
                    <p className="text-text-muted text-xs italic bg-docker-amber/5 rounded-lg p-3 border border-docker-amber/20">
                      {hint}
                    </p>
                  ) : (
                    <button onClick={() => setShowHint(true)} className="text-docker-amber text-xs hover:underline">
                      Show hint
                    </button>
                  )}
                </div>
              ...
              <button
                ...
                className="px-5 py-2 rounded-lg text-sm font-semibold bg-docker-amber/10 text-docker-amber border border-docker-amber/20 hover:bg-docker-amber/20 transition-all mt-2"
              >
                Tap to reveal
              </button>
            </motion.div>
